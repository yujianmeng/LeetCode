...
/**
 * Created by yujian on 2017/11/25.
 * 问题描述
...
public class Route {
    ...
    static List<String> lus = new LinkedList<>();
    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);
        int p = scanner.nextInt();
        String nowStand = new String(scanner.next());
        String[] now = nowStand.split("/");
        if (now.length>0) {
            l = now[now.length - 1].length();
        }
        else {
            nowStand="/";
        }
        ...
        while (i<p){
            ...
            //绝对路径
            if (lu.startsWith("/")){
                getLu(new StringBuffer("/"),lu);
            }
            //相对路径
            else {
                StringBuffer jueLu = new StringBuffer(nowStand).append("/");
                getLu(jueLu,lu);
            }
            i++;
        }
    ...
    private static void getLu(StringBuffer jueLu,String lu){
        String[] nodes = lu.split("/");
        int upperLen = 1;
        if (jueLu.length()>2) {
            upperLen = l+1;
        }
        for (String no:nodes){
            if (!no.equals("")) {
                if (no.equals(".")) {
                    ...
                    if (jueLu.length() != 1) {
                        jueLu = jueLu.delete(jueLu.length() - upperLen,jueLu.length());
                    }
                } else {
                    jueLu.append(no + "/");
                    upperLen = no.length() + 1;
                }
            }
        }
        if (jueLu.length()==1){
            lus.add(jueLu.toString());
        }
        else {
            lus.add(jueLu.substring(0,jueLu.length()-1));
        }
    }
}
