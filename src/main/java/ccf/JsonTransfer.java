...
/**
 * Created by yujian on 2017/10/31.
...
public class JsonTransfer {
    ...
    public static void main(String[] args) {
        ...
        i = 0;
        while (i++ < m) {
            query.add(scanner.nextLine().trim());
        }
        queue.poll();
        while (queue.size() > 0) {
            char tmp = queue.poll();
            if (tmp == '\"') {
                ...
                object = keyAndValue.substring(0, keyAndValue.trim().length() - 1).trim();
        ...
        for (int j = 0; j < query.size(); j++) {
            if (jsonMap.get(query.get(j)) == null) {
                System.out.println("NOTEXIST");
            } else if (jsonMap.get(query.get(j)).equals("OBJECT")){
                System.out.println("OBJECT");
            }else {
                System.out.println("STRING " + jsonMap.get(query.get(j)));
            }
        }
    }

    public static void getJsonData(char tmp) {
        if (tmp == ',' || queue.size() == 0) {
            String[] keyValue = keyAndValue.split(":");
            if (object.equals("")){
                jsonMap.put(keyValue[0].trim(), keyValue[1].trim());
            }else {
                jsonMap.put(object+"."+keyValue[0].trim(), keyValue[1].trim());
            }
            keyAndValue = "";
        } else {
            if (tmp == '\\') {
                keyAndValue = keyAndValue + queue.poll();
            } else {
                keyAndValue = keyAndValue + tmp;
            }
        }
    }
}
